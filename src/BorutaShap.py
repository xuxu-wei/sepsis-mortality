...
class BorutaShap:
    ...
    def leaderboard(self):
        """
        Return a DataFrame with feature importance, p-values, and decision status.

        Returns
        -------
        DataFrame
            A DataFrame containing feature importance and p-value information.
        """
        ...
        p_values_df = pd.DataFrame.from_dict(self.p_values, orient='index').reset_index()
        p_values_df.rename(columns={'index': 'Features'}, inplace=True)

        # Merge feature importance and p-values
        features = features.merge(p_values_df, on='Features', how='left')

        # Add decision (accepted, rejected, tentative)
    ...
    def test_features(self, iteration):
        ...
        self.p_values = {
            col: {
                ...
                "corrected_acceptance_p": corrected_accept,
                "raw_reject_p": raw_reject,
                "corrected_reject_p": corrected_reject,
            }
            for col, raw_accept, corrected_accept, raw_reject, corrected_reject in zip(
                self.columns,
                acceptance_p_values,
                corrected_acceptance_p_values,
                reject_p_values,
